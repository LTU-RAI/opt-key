"""
    This is a class to handle the NewerCollege dataset.
"""
...
class NewerCollege_Handler:
    def __init__(self, path_to_dataset:str, 
                       verbose:bool=False):
        """
            This is a class to handle the NewerCollege dataset, containing any number of sequences.
            Args:
                path_to_dataset: str, path to the NewerCollege dataset
                sequences: List[str], list of sequences to be used
                verbose: bool, whether to print information or not
            Note:
                The NewerCollege dataset is expected to have the following structure:
                path_to_dataset
                ├─ NewerCollege
                |   |── collection 1-newer college
                |   |   |── cam_calibration
                |   |   |   |── cam0-1
                |   |   |   |── ...
                |   |   |── quad-easy
                ...
                |   |   |   |   |── gt-nc-quad-easy.csv
                |   |   |   |── pcds
                |   |   |   |   |── 1625132068-858555136.pcd
                |   |   |   |── ...
                |   |── collection 2-newer college
                |   |── cam_calibration
                |   |   |   |── cam0-1
                |   |   |   |── ...
                |   |   |── cloister
    ...
    def load_newer_college(self, sequence:str, session:str):
        """ Read the NewerCollege dataset and return the pointclouds, poses and timestamps.
        ...
        poses, pose_timestamps = self.load_poses(self.path_to_dataset + f'/{sequence}/{session}/ground_truth/tum_format/gt-nc-{session}.csv')
        ## Load the pointclouds
        scans, scan_timestamps = self.load_scans(self.path_to_dataset + f'/{sequence}/{session}/pcds')
        ## Load the depth maps
    ...
    ## Load the NewerCollege ground truth poses.
    def load_poses(self, pose_path:str) -> np.ndarray:
        ...
        timestamps = []
        try:
            if '.csv' in pose_path or '.txt' in pose_path:
    ...
    ## Load the NewerCollege depth maps.
    ...
    ## Load the NewerCollege point cloud scans.
    def load_scans(self, pointcloud_path:str) -> List:
        """ Load the pointclouds from the NewerCollege dataset. The format is .pcd files.
            Args:
    ...
    ## Load te NewerCollege point cloud scan.
    def load_scan(sellf, pointcloud_file:str) -> np.ndarray:
        """ Load a pointcloud from the NewerCollege dataset. The format is .bin files.
            Args:
    ...
    def sync_data(self, pose_timestamps:np.ndarray, scan_timestamps:np.ndarray) -> np.ndarray:
        ...
        ## Return the synchronized indices 
        return idx

    ## Visulize the NewerCollege ground truth poses.
    ...
    ## Visualize a NewerCollege scan.
    def plot_scan(self, scan:np.ndarray, save:bool=False) -> None:
        """ Visualize a NewerCollege scan using matplotlib.
            Args:
        ...
        ax.set_ylabel('Y [m]')
        ## Set the title
        ax.set_title('Newer-College scan')
        ## Add grid
        plt.grid(alpha=0.5, linestyle='--', linewidth=1.0)
        ## Add limit
        plt.xlim(-100, 100)
    ...
    ## Visualize a NewerCollege pointcloud.
    def visualize_scan(self, scan:np.ndarray) -> None:
        """ Visualize a NewerCollege pointcloud using open3d.
            Args:
                scan: A numpy array of size nx4 with n points as 3D points with homogeneous coordinates.
            Returns:
                None
...
#     ## Define the path to the NewerCollege dataset
#     path_to_dataset = '/home/niksta/Documents/datasets/NewerCollege/'
#     ## Create an instance of the NewerCollege_Handler class
#     nc_handler = NewerCollege_Handler(path_to_dataset, verbose=True)

#     ## Load the NewerCollege dataset
#     poses, pose_timestamps, scans, scan_timestamps = nc_handler.load_newer_college('collection 2-newer college', 'park')
#     ## Synchronize the data
#     indices = nc_handler.sync_data(pose_timestamps, scan_timestamps)
...
#     nc_handler.visualize_poses(poses, save=False)
#     ## Plot the first scan
#     nc_handler.plot_scan(scans[0], save=False)
